...
(
    ...
    200.00, 100.00, 100,
    NOW(), NOW() + INTERVAL '30 days', '{"Tandoori", "Fried", "Rolls"}', true, true
),
(
    'FLAT50', 'Flat ₹50 Off', 'Get ₹50 off on orders above ₹300', 'fixed_amount', 50.00,
    300.00, 50.00, 200,
    NOW(), NOW() + INTERVAL '60 days', '{}', false, false
),
(
    'SPICY20', 'Spicy Special', '20% off on all spicy items', 'percentage', 20.00,
    150.00, 80.00, 50,
    NOW(), NOW() + INTERVAL '15 days', '{"Tandoori", "Fried"}', false, true
)
...
GRANT SELECT ON discount_coupons TO authenticated;
GRANT SELECT ON coupon_usage TO authenticated;
GRANT SELECT ON orders TO authenticated;
GRANT SELECT ON customers TO authenticated;

-- 14. Verify setup
COMMENT ON TABLE coupon_usage IS 'Tracks coupon usage by customers for security and analytics';
COMMENT ON TABLE discount_coupons IS 'Stores discount coupons with validation rules';
COMMENT ON FUNCTION validate_coupon_usage IS 'Server-side coupon validation for security';

-- IMPORTANT: After running this script, test the coupon system:
-- 1. Try applying a coupon without selecting a customer - should show error
-- 2. Select a customer and apply coupon - should work
-- 3. Test first-time customer restriction
-- 4. Test single-use per customer restriction
-- 5. Test minimum order amount restriction
